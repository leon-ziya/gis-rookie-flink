...
/**
 * 同步的Http请求对象，支持post与get方法一支持可设置代理
 * @author leon
 * @ClassName HttpClientSyncUtil.java
 * @createTime 2020年10月16日 17:51:00
 */
public class HttpClientSyncUtil {
    ...
    private Logger logger = LoggerFactory.getLogger(HttpClientSyncUtil.class);
    // proxy代理相关配置
    private String host = "";
    private int port = 0;
    ...
    public String httpPost(String url, List<BasicNameValuePair> parameters,
        ...
        try {
            ...
            logger.error(Thread.currentThread().getName() + "httpPost Send Error,Code error:" + e.getMessage());
        } catch (ClientProtocolException e) {
            logger.error(Thread.currentThread().getName() + "httpPost Send Error,Protocol error:" + e.getMessage());
        } catch (IOException e) {
            logger.error(Thread.currentThread().getName() + "httpPost Send Error,IO error:" + e.getMessage());
        } catch (URISyntaxException e) {
            logger.error(Thread.currentThread().getName() + "httpPost Send Error,IO error:" + e.getMessage());
        } finally {// 释放连接,将连接放回到连接池
